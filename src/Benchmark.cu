                        ...
                        ***********************************************************************************************/
                        ...
                        for (int j = 0; j < intervalMAX; j++) {
                            ...
                            if (j >= WARMUP && !(*failFlag)) {
                                ...
                                myfile << N << "," << numThreads << "," << P << "," << D << "," << S << ",ccuc," << (int)tableCount << "," << (std::chrono::duration_cast<std::chrono::nanoseconds> (end - begin).count()) << ", INS,\n";
                            }
                            if (*failFlag) {
                                //printf("\t\t\t\t\t\tFailed\n");
                                myfile << N << "," << numThreads << "," << P << "," << D << "," << S << ",ccuc," << (int)tableCount << "," << -1 << ",INS,\n";
                                break;
                            }
