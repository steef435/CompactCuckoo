...
inline uint64_t rshft(const uint64_t x, uint8_t i) {
	return (x >> i);
}

// Bit left shift function for 58 bits.
inline  uint64_t lshft_58(const uint64_t x, uint8_t i) {
	uint64_t y = (x << i);
	return y & 0x3ffffffffffffff;
}

// Multiplication modulo 2^58.
inline  uint64_t mult_58(const uint64_t x, uint64_t a) {
	return ((x * a) & 0x3ffffffffffffff);
}

// XOR two times bit shift function for 58 bits.
inline  uint64_t xor_shft2_58(const uint64_t x, uint8_t a, uint8_t b) {
	uint64_t y = (x ^ lshft_58(x,a));
	y = (y ^ rshft(y,b));
	return y;
}

inline nodetype RHASH(uint8_t id, nodetype node) {
	nodetype node1 = node;
	switch (id) {
		case 0:
...
inline  nodetype RHASH_INVERSE(uint8_t id, nodetype node) {
	nodetype node1 = node;
	nodetype node2;
	switch (id) {
		case 0:
